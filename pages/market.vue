<template>
  <div class="relative bg-white sm:bg-bgsecondary overflow-hidden">
    <div
      ...
      bg-[length:70%] bg-no-repeat sm:bg-cover bg-bgsecondary sm:bg-bgcolor bg-left-bottom sm:bg-right"
        ...
        >
          ...
          <div>
            <div class="text-center mb-8">
              ...
              <p>為你想要的未來買單</p>
              <p>逛逛自綠商店街，來趟綠色消費之旅！</p>
            </div>
            
            <ul class="flex flex-wrap">
              <li
                v-for="(list, index) in market"
                :key="index"
...
const activeData = computed(() => {
  return market.value[active.value]
})

const { height, isMobile } = useClientSize()
const backdrop = new URL(`../assets/images/market-backdrop.svg`, import.meta.url).href
const backdropM = new URL(`../assets/images/market-backdrop-m.svg`, import.meta.url).href

const error = ref(false)
const { publicPath } = useClientConfig()
const fetchUrl = `${publicPath}static/data/market.json?${new Date().getTime()}`
